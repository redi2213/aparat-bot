...
def record_history(user_id, source, file_name, file_size, link):
    """
    Tells the Worker to add a completed job to the user's history in KV.
    Actions has no direct access to Cloudflare KV, so this is the bridge —
    a simple authenticated POST to a Worker endpoint that does the actual
    KV write (see worker/index.js handleHistoryAdd).
...
def notify_done(chat_id, message, link=None):
    ...
    if link:
        text += f"\n\n🔗 {link}"
    notify(chat_id, text, reply_markup=BACK_TO_MENU_MARKUP)
