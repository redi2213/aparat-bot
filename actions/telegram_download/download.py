...
def stage_run_upload():
    ...
    updates purely so the user can tell the job hasn't silently died, with
    a warning if it's taking unusually long.
    """
    ...
    heartbeat = HeartbeatReporter(CHAT_ID, label="در حال آپلود") if CHAT_ID else None
    if heartbeat:
        heartbeat.start()
