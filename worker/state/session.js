 ...
 * @property {"internal"|"external"} storage - which final storage the user picked: "internal" = a provider from storageProviders.js (ArvanCloud etc), "external" = GitHub Release (the original/default path). Defaults to "external" so behaviour is unchanged for anyone who never touches the new option.
 * @property {"awaiting_name"|"menu"|"awaiting_add_user_id"|"awaiting_remove_user_id"|"awaiting_provider_key"|"awaiting_quota_input"|"confirmed"} stage
 * @property {string} [pendingProviderId]  - set while stage === "awaiting_provider_key": which provider (see storageProviders.js) the key currently being typed belongs to
 * @property {"access"|"secret"|"bucket"} [pendingKeyField] - which piece of the provider key set we're currently collecting (asked for one at a time, in sequence)
 * @property {Object} [collectedKeyParts]  - accumulates accessKey/secretKey/bucket as the user answers each prompt in turn; written to the user/admin record only once the whole set is collected
 * @property {"internal"|"external"} [pendingQuotaKind] - set while stage === "awaiting_quota_input": which quota the admin is currently typing a number (in GB) for
 * @property {string} [pendingQuotaTargetId] - the user id whose quota is being edited
