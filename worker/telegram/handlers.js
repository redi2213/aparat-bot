...
async function startJob(env, userId, chatId, messageId, session, ctx) {
  ...
  // GitHub's repository_dispatch API hard-caps client_payload at 10
  // top-level properties ("No more than 10 properties are allowed") —
  // the base telegram payload alone is 7, so the storage_* fields (6-7
  // more) must be nested under one wrapper property rather than spread at
  // the top level, or dispatch silently gets rejected before the Action
  // ever runs. See .github/workflows/*.yml, which reads these back out
  // via github.event.client_payload.storage.* instead of flat storage_*
  // keys.
  const clientPayload =
...
async function handleStorageFailureChoice(env, callback, userId, chatId, messageId, data, ctx) {
  ...
  if (data === "storage_fail_fallback_github") {
    clientPayload.storage = { storage_target: "external" };
  }
  // storage_fail_retry: clientPayload.storage already has the same
  // internal fields from the original attempt, so nothing to change.
  ...
  await setActiveJob(env, userId, {
    ...
    messageId,
    startedAt: Date.now(),
    storage: clientPayload.storage && clientPayload.storage.storage_target === "internal" ? "internal" : "external",
